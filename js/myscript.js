	...
	function fetching()
	{
		...
		self.list().forEach(function(x)
		{
			...
			$.getJSON(fsqURL).done(function(data)
			{
				...
				var num = response.venue.contact.phone; 
				num ? x.phoneNo = num : x.phoneNo = 'No phone Number available';
				var addr = response.venue.location.formattedAddress;
				addr ? x.address = addr : x.address = 'Address not available';
